		...
		this.usps = function(tracking_number) {
			...
			var parseTime = function (date, hour) {
				var parseHour = function (hour) {
					var timeArray = hour.split(/[:\ ]/);
					if (timeArray[2] === 'am') {
						if (timeArray[0] === '12') {
							return Number(timeArray[1])*60;
						}
						return Number(timeArray[0])*60*60 + Number(timeArray[1])*60;
					} else {
						if (timeArray[0] === '12') {
							return 43200 + Number(timeArray[1])*60;
						}
						return Number(timeArray[0])*60*60 + 43200 + Number(timeArray[1])*60;
					}
				};
				var dateInMilliseconds = Date.parse(date);
				var hourInMilliseconds = hour ? parseHour(hour) * 1000 : 0;
				var seconds = new Date(dateInMilliseconds + hourInMilliseconds).toISOString();
				return seconds.substr(0, seconds.length-5);
			};
